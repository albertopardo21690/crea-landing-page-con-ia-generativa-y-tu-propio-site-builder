...
class CodeController{
    ...
    public function ajaxCode(){
		...
		if(file_put_contents($folderPath, file_get_contents($this->img))){
			...
			$url = "landings?id=".$this->idCode."&nameId=id_landing&token=".$this->token."&table=admins&suffix=admin";
			$method = "PUT";
			...
			if($landing->status == 200){
				...
				$url = "codes?id=".$this->idCode."&nameId=id_landing_code&token=".$this->token."&table=admins&suffix=admin";
		     	$method = "PUT";
    ...
    public function deleteLanding(){
    	...
    	$url = "codes?id=".$this->idDelete."&nameId=id_landing_code&token=".$this->token."&table=admins&suffix=admin";
    	$method = "DELETE";
    	$fields =  array();
    	...
    	if($deleteCode->status == 200){
            ...
            $url = "landings?id=".$this->idDelete."&nameId=id_landing&token=".$this->token."&table=admins&suffix=admin";
	    	$method = "DELETE";
	    	$fields =  array();
